...
namespace Game
{
    public class UIDialogue : UIWindow
    {
        ...
        private IEnumerator SetTextTask(string text)
        {
            ...
            while (len < lineBreakedText.Length)
            {
                ...
                else
                {
                    ...
                    //환경설정에서 적용한 텍스트 이미지 출력속도를 적용 (원래 적용한 텍스트 속도가 제일 빠름)
                    int count = (int)Mathf.Max(1.0f, deltaTime / (Define._textInterval)* float.Parse(PlayerPrefs.GetString("textSpeed")));
                ...
                yield return new WaitForSeconds(Define._textInterval/float.Parse(PlayerPrefs.GetString("textSpeed")));
        ...
        private void StartBlinkWaitIcon() => _blinkWaitIconCoroutine = StartCoroutine(BlinkWaitIcon());
