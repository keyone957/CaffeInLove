...
public abstract class UIManager : MonoBehaviour
{
    ...
    public UIWindow OpenWindow(string prefabPath) => OpenWindow(prefabPath, _Trans);
        ...
        where T : MonoBehaviour
    {
        ...
        if (rectTrans)
            ...
            Debug.LogError($"[UIManager] Created widget doesn't have RectTransform. prefabWidget: {prefabWidget.name}");

        T widgetComponent = objWidget.GetComponent<T>();
        if (!widgetComponent)
            Debug.LogError($"[UIManager] Created widget doesn't have target component. prefabWidget: {prefabWidget.name}, component type:{typeof(T).Name}");
    ...
    protected virtual bool OnKeyInputOnComponent() => false;
        ...
        if (_windows.Count > 0)
        {
            ...
            for (int i = _windows.Count - 1; i >= 0; --i)
            {
                ...
                if (window != null && window.OnKeyInput())
                    return true; // 키입력 전달하지 않는 윈도우 발생
            }
        }
